...
class Cart extends Component{
    ...
    componentDidUpdate(){
        ...
        for(let i =0 ;i<arr.length;i++){
            jian[i].onclick = ()=>{
                arr[i].num--
                arr[i].zongjia= arr[i].num*arr[i].max_app_price
				...
				<div className="cart_content">
				{
					this.state.shopType?<ul>
					{
						this.state.shopArr.map((item,index)=><li key={index}>
                            ...
                            <button className="jian" onClick={this.jian}>-</button><span>{item.num}</span><button className="add" onClick={this.add}>+</button>
                            <span>{item.zongjia}</span>
						</li>)
					}
